...
RSpec.describe "Admin dashboard async slices", type: :request do
  ...
  it "renderiza desempenho comercial no período selecionado" do
    ...
    open_lead.activities.create!(tenant: admin.tenant, kind: "note", source_category: "external_sync", metadata: { contact_kind: "ligacao", contact_result: "falou_com_cliente", body: "Evento externo não deve entrar nas tentativas." })
    ...
    expect(response.body).to include("Histórico de contatos")
    expect(response.body).not_to include('title="Tentativas')
    ...
    expect(response.body).not_to include("Evento externo não deve entrar nas tentativas.")
