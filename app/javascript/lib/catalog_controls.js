...
import {formatCurrencyFilter,currencyFilterDigits} from './currency_filter.js';
...
const input=(name,label,{prefix='',placeholder='',type='text'}={})=>`<label>${label}<span class="pc-input-group">${prefix?`<span>${prefix}</span>`:''}<input name="${name}" type="${type}" ${prefix==='R$'?'data-price inputmode="decimal" maxlength="24"':type==='number'?'min="0" step="any"':''} placeholder="${placeholder}"></span></label>`;
const rangeInputs=(key,label,options={})=>`<div class="pc-range-pair">${input(`${key}_min`,`${label} mín.`,options)}${input(`${key}_max`,`${label} máx.`,options)}</div>`;
...
const checks=(name,options)=>`<div class="pc-checks">${options.map(([v,t])=>`<label><input type="checkbox" name="${name}" value="${escape(v)}">${escape(t)}</label>`).join('')}</div>`;
...
export function filterMarkup(items=[],options={}){
 ...
 ].map(([key,label])=>rangeInputs(key,label,{type:'number'})).join('')+'<strong>CARACTERÍSTICAS INTERNAS</strong>'+checks('amenities',(options.amenity_features || []).map(value=>[value,value]))+'<strong>CARACTERÍSTICAS DO EMPREENDIMENTO</strong>'+checks('amenities',(options.amenity_infrastructure || []).map(value=>[value,value])),true)}
 </div>`;
}
// Reuse the admin's whole-reais mask; never append cents on blur.
export function maskCatalogPrice(value,complete=false){
 return formatCurrencyFilter(String(value??'').split(',')[0]);
}
export function catalogPriceValue(value){
 return currencyFilterDigits(String(value??'').split(',')[0]);
}
export function enhanceFilters(form){
 for(const el of form.querySelectorAll('[data-price]')){
  el.oninput=()=>{
   const before=el.value,caret=el.selectionStart;
   const count=before.slice(0,caret).replace(/\D/g,'').length;
   el.value=maskCatalogPrice(before);
   let position=0,seen=0;
   while(position<el.value.length&&seen<count){if(/\d/.test(el.value[position]))seen++;position++;}
   el.setSelectionRange(position,position);
  };
  el.onblur=()=>{el.value=maskCatalogPrice(el.value,true);};
 }
...
export function restoreFilters(form,values={}){
 ...
 for(const el of form.querySelectorAll('input,select')){
  ...
  else if(el.hasAttribute('data-price'))el.value=maskCatalogPrice(/^\d+\.\d{1,2}$/.test(String(value))?String(value).split('.')[0]:value);
  else el.value=value;
 }
}
export function readFilters(form){
 ...
 for(const key of ['min_price','max_price'])if(key in result)result[key]=catalogPriceValue(result[key]);
 return result;
}
