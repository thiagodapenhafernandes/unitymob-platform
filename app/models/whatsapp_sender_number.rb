class WhatsappSenderNumber < ApplicationRecord
  ...
  def self.default_for_campaign
    active.ordered.first || sync_from_current_integration!
  end

  def self.sync_from_current_integration!
    integration = WhatsappBusinessIntegration.current
    return nil unless integration.phone_number_id.present?

    find_or_create_by!(phone_number_id: integration.phone_number_id) do |number|
  ...
  def access_token
    whatsapp_business_integration&.access_token || WhatsappBusinessIntegration.current.access_token
  end
