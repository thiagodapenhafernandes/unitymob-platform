class Admin::AccessSecurityController < Admin::BaseController
  ...
  def load_dashboard
    rules_scope = AccessControlRule.includes(:profile, :admin_user, :created_by).recent
    ...
    devices_scope = TrustedDevice.includes(admin_user: :profile).includes(:created_by).recent
    ...
    @available_device_types = TrustedDevice.where.not(device_type: [nil, ""]).distinct.order(:device_type).pluck(:device_type)
    @available_browsers = TrustedDevice.where.not(browser: [nil, ""]).distinct.order(:browser).pluck(:browser)
