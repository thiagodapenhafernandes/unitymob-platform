module Admin
  class CaptacoesController < Admin::BaseController
    ...
    def dashboard
      ...
      @meta_venda   = CaptacaoGoal.current_target(start_date: @period_start, end_date: @period_end, kind: :venda, tenant: current_tenant)
      @meta_locacao = CaptacaoGoal.current_target(start_date: @period_start, end_date: @period_end, kind: :locacao, tenant: current_tenant)
      ...
      rental_management_column = Habitation.physical_column_name(:rental_management_flag)
      @ranking_locacao = locacao_scope
        ...
        .select(
          ...
          "SUM(CASE WHEN habitations.#{rental_management_column} = TRUE THEN 1 ELSE 0 END) AS adm_ct, " \
          ...
          "SUM(CASE WHEN habitations.#{rental_management_column} = TRUE AND habitations.exibir_no_site_flag = TRUE THEN 1 ELSE 0 END) AS adm_site_ct, " \
          "SUM(CASE WHEN habitations.#{rental_management_column} = TRUE AND habitations.exibir_no_site_flag = TRUE THEN 0 WHEN habitations.#{rental_management_column} = TRUE THEN 1 ELSE 0 END) AS adm_internal_ct, " \
          "COALESCE(SUM(habitations.valor_locacao_cents),0) / 100.0 AS total_value"
        )
        .order("ct DESC, total_value DESC")
        .limit(15)

      @goal_venda_obj   = CaptacaoGoal.current_foco(start_date: @period_start, end_date: @period_end, kind: :venda, tenant: current_tenant)
      ...
      @captacao_adm_locacao = locacao_scope.where(Habitation.physical_column_name(:rental_management_flag) => true).count
