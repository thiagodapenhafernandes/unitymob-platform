...
module Habitations
  class HierarchyAuditService
    def initialize(strict: false, tenant: nil)
      @strict = strict
      @tenant = tenant || Current.tenant
      raise ArgumentError, "Tenant obrigatório para auditar hierarquia de imóveis" if @tenant.blank?
    end
    ...
    def metrics
      linked_units = habitation_scope.where.not(codigo_empreendimento: [nil, ""])
      developments = habitation_scope.empreendimentos
    ...
    def duplicate_development_codes_count
      habitation_scope.empreendimentos
    ...
    def units_constructor_diff_from_parent_relation
      habitation_scope.where.not(codigo_empreendimento: [nil, ""])
                      .joins("INNER JOIN habitations dev ON dev.codigo = habitations.codigo_empreendimento AND dev.tenant_id = habitations.tenant_id")
