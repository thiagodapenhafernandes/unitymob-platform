module Habitations
  class AmenityFilter
    ...
    def sea_view
      @scope.where(
        "habitations.vista_frente_mar_flag IS TRUE OR " \
        "(jsonb_typeof(habitations.caracteristicas) = 'array' AND EXISTS (SELECT 1 FROM jsonb_array_elements_text(habitations.caracteristicas) value WHERE unaccent(value) ILIKE unaccent('%vista%mar%'))) OR " \
        "(jsonb_typeof(habitations.caracteristicas) = 'object' AND EXISTS (SELECT 1 FROM jsonb_each_text(habitations.caracteristicas) kv WHERE unaccent(kv.key) ILIKE unaccent('%vista%mar%') OR unaccent(kv.value) ILIKE unaccent('%vista%mar%'))) OR " \
        "EXISTS (SELECT 1 FROM unnest((#{Habitation::SearchScopes::UNIQUE_FEATURES_ARRAY_SQL})) AS feature WHERE unaccent(feature) ILIKE unaccent('%vista%mar%'))"
      )
    end
