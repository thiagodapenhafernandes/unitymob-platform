module Vista
  class FileAssetHabitationLinkService
    ...
    def initialize(batch: VistaImportBatch.latest_first.first, dry_run: true, tenant: Current.tenant)
      @batch = batch
    ...
    def call
      ...
      raise ArgumentError, "Tenant obrigatório para vincular anexos Vista" unless @tenant
    ...
    def matching_scope
      VistaFileAsset
        .joins("INNER JOIN habitations ON habitations.codigo = vista_file_assets.codigo_imovel")
    ...
    def link_assets!
      ...
      quoted_tenant_id = ActiveRecord::Base.connection.quote(@tenant.id)
      ...
      ActiveRecord::Base.connection.exec_update(<<~SQL.squish, "Vista file asset habitation link")
        UPDATE vista_file_assets
         ...
         WHERE habitations.codigo = vista_file_assets.codigo_imovel
           AND habitations.tenant_id = #{quoted_tenant_id}
